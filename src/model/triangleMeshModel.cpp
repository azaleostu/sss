...
void TriangleMeshModel::load(const std::string& p_name,
  ...
  _name = p_name;
  std::cout << "Loading model " << _name << " from: " << p_filePath
            << std::endl;
  ...
  if (scene == nullptr) {
    throw std::runtime_error("Fail to load file \" " + p_filePath.str() +
  ...
  std::partition(_meshes.begin(), _meshes.end(),
                 [](const TriangleMesh& p_mesh) -> bool {
                   return p_mesh._material._opaque;
                 });
...
void TriangleMeshModel::_loadMesh(const aiMesh* const p_mesh,
  ...
  if (mtl == nullptr) {
    ...
    std::cerr << "=> Material undefined," << meshName
              << " assigned to default material" << std::endl;
  } else {
...
Material TriangleMeshModel::_loadMaterial(const aiMaterial* const p_mtl) {
  ...
  } else if (p_mtl->Get(AI_MATKEY_COLOR_AMBIENT, color) ==
             AI_SUCCESS) // else Material ?
  {
  ...
  } else if (p_mtl->Get(AI_MATKEY_COLOR_DIFFUSE, color) ==
             AI_SUCCESS) // else Material ?
  {
  ...
  } else if (p_mtl->Get(AI_MATKEY_COLOR_SPECULAR, color) ==
             AI_SUCCESS) // else Material ?
  {
  ...
  } else if (p_mtl->Get(AI_MATKEY_SHININESS, shininess) ==
             AI_SUCCESS) // else Material ?
  {
  ...
  } else if (p_mtl->Get(AI_MATKEY_TEXTURE(aiTextureType_NORMALS, 0), normal) ==
             AI_SUCCESS) // else Material ?
  {
    material._normal = normal;
  }
  // =====================================================
...
Texture TriangleMeshModel::_loadTexture(const aiString& p_path,
  ...
  if (image.load(fullPath)) {
    ...
    int mipmaplvl = log2(glm::max(image._width, image._height));
    glTextureStorage2D(texture._id, mipmaplvl, internalFormat, image._width,
                       image._height);
    glTextureParameteri(texture._id, GL_TEXTURE_WRAP_S, GL_REPEAT);
    glTextureParameteri(texture._id, GL_TEXTURE_WRAP_T, GL_REPEAT);
    glTextureParameteri(texture._id, GL_TEXTURE_MIN_FILTER,
                        GL_LINEAR_MIPMAP_LINEAR);
    glTextureParameteri(texture._id, GL_TEXTURE_MAG_FILTER, GL_LINEAR);

    // Fill the texture.
    glTextureSubImage2D(texture._id, 0, 0, 0, image._width, image._height,
                        format, GL_UNSIGNED_BYTE, image._pixels);
